"""
MCTS with Softmax Probabilistic Selection Module

This module implements a Monte Carlo Tree Search (MCTS) player that evaluates each
possible card move by simulating random rollouts, then selects cards based on a 
probability distribution derived from the evaluations. Unlike greedy selection, this
approach allows the agent to occasionally play suboptimal moves, enabling exploration
and adaptability.
...
- Softmax probability sampling: converts scores to probabilities using temperature
- Tunable exploration: temperature controls exploration vs. exploitation trade-off
...
class MCTS():
    """
    Monte Carlo Tree Search player with softmax probabilistic card selection.
    ...
    The final card selection uses softmax probability distribution to balance
    exploitation (choosing good cards) with exploration (trying different moves).
    """
    ...
    def __init__(self, player_idx):
        ...
        self.temperature = 0.5  # Controls exploration: lower = more greedy, higher = more random
        ...
        for turn in range(turns_left):
            ...
            for card, owner in played_cards:
                ...
                if not valid_rows:
                    ...
                    # Player must take one row - choose the cheapest (minimum bullheads)
    ...
    def action(self, hand, history):
        """
        Select the best card to play using MCTS with softmax probabilistic selection.
        ...
        5. Convert scores to probabilities using softmax with temperature
        6. Sample card from probability distribution
        ...
        Returns:
            int: The selected card to play (sampled from probability distribution)
        """
        ...
        # Step 5: Softmax Probability Sampling
        # Calculate scores for all cards
        scores_array = np.array([calc_score(k) for k in hand], dtype=np.float32)
        
        # Shift scores for numerical stability (subtract min)
        scores_shifted = scores_array - np.min(scores_array)
        
        # Apply softmax with temperature: P(card) = exp(-score/T) / sum(exp(-score/T))
        # Lower scores (better cards) have higher probability
        exponents = -scores_shifted / self.temperature
        probabilities = np.exp(exponents)
        probabilities = probabilities / np.sum(probabilities)
        
        # Sample card based on probability distribution
        selected_card = np.random.choice(hand, p=probabilities)
