...
class FlatMCBelief:
    ...
    def __init__(self, player_idx, c_param=5.0, epsilon=0.1, belief_model_path=None):
        """
        ...
        Args:
            ...
            belief_model_path (str | None): Optional path to a trained BeliefPPO model for opponent hand probability prediction.
        """
        ...
        if belief_model_path:
            ...
            if os.path.exists(f"{belief_model_path}.zip"):
                self.model = BeliefPPO.load(belief_model_path)
                ...
                print(f"Warning: Belief model not found at {belief_model_path}. Falling back to uniform sampling.")
    ...
    def action(self, hand, history):
        ...
        belief_probs = None
        if self.model is not None and isinstance(history, dict):
            ...
            unseen = self.compute_unseen_cards(hand, board, history_matrix, board_history)
            obs = self.extract_features(
                board=board, hand=hand, unseen=unseen, scores=scores,
                player_idx=self.player_idx, round_num=round_num,
            ...
            with th.no_grad():
                ...
                logits = self.model.policy.belief_head(features)
                belief_probs = th.sigmoid(logits).cpu().numpy().flatten()
        ...
        while time.perf_counter() - start_time < self.time_limit:
            ...
            if belief_probs is not None:
                # Gumbel-Max trick for vectorized weighted sampling without replacement
                U = np.clip(np.random.rand(actual_batch_size, 105), 1e-10, 1.0 - 1e-10)
                gumbel_noise = -np.log(-np.log(U))
                rand_weights = np.log(belief_probs + 1e-10) + gumbel_noise
            else:
                rand_weights = np.random.rand(actual_batch_size, 105)
                
            unseen_mask = np.tile(unseen_mask_base, (actual_batch_size, 1))
            rand_weights[~unseen_mask] = -100.0  # Safe minimal value for log-space
