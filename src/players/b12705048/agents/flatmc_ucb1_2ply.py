"""
UCB1 Monte Carlo (2-Ply) Player Module — Vectorized SoA Variant.

This module implements a 2-ply Monte Carlo agent that uses the UCB1 bandit
algorithm to dynamically allocate simulation budget across pairs of candidate 
cards (a1, a2). It combines this with an epsilon-greedy min-max rollout policy 
for the remaining turns.

Algorithm:
    1. If only 1 card remains in hand, return it trivially.
    2. Build all valid 2-card sequences (a1, a2) as candidates.
    3. Allocate simulation budget per candidate pair using UCB1.
    4. Build SoA arrays for `batch_size` independent games.
    5. Randomly assign unseen cards to opponents via vectorized argsort.
    6. For our player, assign (a1, a2) to the first two tricks, and use the 
       epsilon-greedy min-max policy for the remainder of the hand.
    7. Simulate all tricks across all games simultaneously using NumPy.
    8. Repeat until the wall-clock budget expires.
    9. Select the optimal a1 that minimizes the expected penalty assuming 
       we make the best choice for a2: argmin_a1 min_a2 AvgPenalty(a1, a2).
"""
...
class FlatMCUCB1_2Ply:
    """
    Vectorized 2-ply Monte Carlo agent using UCB1 for dynamic exploration
    of 2-card sequences.
    ...
    def __init__(self, player_idx, c_param=5.0, epsilon=0.1, time_limit=0.8):
        """
        Initialize the 2-Ply UCB1 MC player.
    ...
    def action(self, hand, history):
        """
        Evaluate candidate pairs via batched SoA simulation.
        ...
        Returns:
            int: The card a1 that minimizes the min_a2 expected penalty.
        """
        ...
        while time.perf_counter() - start_time < self.time_limit:
            ...
            # Assign our 2-ply candidate cards
            current_start = 0
            for cand in candidates:
                sims_per_cand = budget[cand]
                ...
                c1, c2 = cand
                my_rest = [x for x in hand if x not in cand]
                ...
                if len(my_rest) > 0:
                    ...
                    hands_array[start_b:end_b, self.player_idx, 2:] = final_my
        ...
        # ---- Final Decision: argmin_{a1} min_{a2} AvgPenalty(a1, a2) ----
        best_a1 = None
        best_a1_score = float('inf')
        
        for a1 in hand:
            a1_best_score = float('inf')
            for a2 in hand:
                if a1 == a2: 
                    continue
                cand = (a1, a2)
                visits = stats_visits[cand]
                if visits > 0:
                    score = stats_penalty[cand] / visits
                    if score < a1_best_score:
                        a1_best_score = score
                        
            if a1_best_score < best_a1_score:
                best_a1_score = a1_best_score
                best_a1 = a1
