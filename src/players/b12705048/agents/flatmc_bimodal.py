...
class FlatMCBimodal:
    """
    Vectorized 1-ply Monte Carlo agent simulating bimodal (non-uniform) candidate budgeting. policy.
    ...
    def __init__(self, player_idx, extreme_sim_ratio=0.8):
        """
        Initialize the BimodalMC player.

        Args:
            player_idx (int): The player's seat index in the game (0-3).
            extreme_sim_ratio (float): Fraction of simulation budget dedicated to extreme cards.
        """
        self.player_idx = player_idx
        self.extreme_sim_ratio = extreme_sim_ratio
        self.time_limit = 0.8
        self.epsilon = 0.1
    ...
    def action(self, hand, history):
        ...
        while time.perf_counter() - start_time < self.time_limit:
            ...
            if num_cand <= 2:
                ...
                extreme_cards = [min(candidates), max(candidates)]
                middle_cards = [c for c in candidates if c not in extreme_cards]
                
                total_ext_sims = int(self.batch_size * self.extreme_sim_ratio)
                sims_per_ext = total_ext_sims // 2
                
                total_mid_sims = self.batch_size - (sims_per_ext * 2)
                sims_per_mid = total_mid_sims // len(middle_cards)
                
                for c in candidates:
                    if c in extreme_cards:
                        budget[c] = sims_per_ext
                    else:
                        budget[c] = sims_per_mid
