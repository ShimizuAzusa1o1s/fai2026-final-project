...
def train_agent(test_mode=False, start_stage=1, start_model_path=None):
    ...
    if start_stage <= 1:
        ...
        model.save(f"{model_dir}/rl_model_143_stage1")
        env_stage1.close()
    ...
    if start_stage <= 2:
        ...
        if model is None:
            load_path = start_model_path if start_model_path else f"{model_dir}/rl_model_143_stage1"
        ...
        checkpoint_callback = CheckpointCallback(save_freq=100_000 // n_envs, save_path=model_dir, name_prefix="rl_model_143_stage2")
        model.learn(total_timesteps=steps_2, callback=checkpoint_callback)
        
        print(f"Stage 2 Complete in {time.time() - start_time:.2f}s")
        model.save(f"{model_dir}/rl_model_143_stage2")
        env_stage2.close()
    ...
    if start_stage <= 3:
        ...
        if model is None:
            load_path = start_model_path if start_model_path else f"{model_dir}/rl_model_143_stage2"
            print(f"Loading model from {load_path}...")
        ...
        checkpoint_callback_3 = CheckpointCallback(save_freq=100_000 // n_envs, save_path=model_dir, name_prefix="rl_model_143_stage3")
        model.learn(total_timesteps=steps_3, callback=checkpoint_callback_3)
        
        print(f"Stage 3 Complete in {time.time() - start_time:.2f}s")
        model.save(f"{model_dir}/rl_model_143_stage3")
        env_stage3.close()
    ...
    if start_stage <= 4:
        env_kwargs_stage4 = {
            ...
            "opponent_model_path": "src/players/b12705048/agents/models/rl_model_143_stage1"
        }
        ...
        if model is None:
            load_path = start_model_path if start_model_path else "src/players/b12705048/agents/models/rl_model_143_stage3"
            print(f"Loading model from {load_path}...")
        ...
        checkpoint_callback_4 = CheckpointCallback(save_freq=100_000 // n_envs_selfplay, save_path=model_dir, name_prefix="rl_model_143_stage4")
        model.learn(total_timesteps=steps_4, callback=checkpoint_callback_4)
        
        print(f"Stage 4 Complete in {time.time() - start_time:.2f}s")
        model.save(f"{model_dir}/rl_model_143_stage4")
        env_stage4.close()
    ...
    if start_stage <= 5:
        env_kwargs_stage5 = {
            "opponent_type": "rl_agent",
            "opponent_model_path": "src/players/b12705048/agents/models/rl_model_143_stage2"
        }
        ...
        if model is None:
            load_path = start_model_path if start_model_path else f"{model_dir}/rl_model_143_stage4"
            print(f"Loading model from {load_path}...")
        ...
        checkpoint_callback_5 = CheckpointCallback(save_freq=100_000 // n_envs_selfplay, save_path=model_dir, name_prefix="rl_model_143_stage5")
        model.learn(total_timesteps=steps_5, callback=checkpoint_callback_5)
        
        print(f"Stage 5 Complete in {time.time() - start_time:.2f}s")
        model.save(f"{model_dir}/rl_model_143_stage5")
        env_stage5.close()
